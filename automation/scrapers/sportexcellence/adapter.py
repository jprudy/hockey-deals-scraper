...
def run(run_id: str, output_dir: Path, log_dir: Path) -> Tuple[List[Dict[str, str]], Dict[str, str]]:
    log_path = log_dir / "scraper_sportexcellence.log"
    raw_csv_path = output_dir / "csv" / "raw_sportexcellence.csv"

    env = os.environ.copy()
    env["RUN_ID"] = run_id
    env["OUTPUT_CSV_PATH"] = str(raw_csv_path)

    command_override = env.get("SPORTEXCELLENCE_SCRAPER_COMMAND")
    if command_override:
        command = shlex.split(command_override, posix=(os.name != "nt"))
    else:
        command = [sys.executable, "automation/scrapers/sportexcellence/scrape.py", "--out", str(raw_csv_path)]
